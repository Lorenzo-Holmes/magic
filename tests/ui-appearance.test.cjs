...
test('四种道身外观均有站姿与头像，默认值确定且非法 ID 拒绝',()=>{
  const {A}=runtime();assert.equal(A.characters.length,4);assert.equal(A.read().id,'jade-sword');
  for(const c of A.characters){assert.match(A.image('standing','',c.name,c.id),/assets\/ui-v4\/characters\/.+-standing\.webp/);assert.match(A.image('portrait','',c.name,c.id),/assets\/ui-v4\/characters\/.+-portrait\.webp/);}
...
test('V4 正式包包含八张角色图并退役单男性与旧洞府分层',()=>{
  const files=require('../tools/production-files.cjs'),m=require('../assets/ui-v4/manifest.json');
  assert.equal(m.assets.length,8);for(const a of m.assets)assert.ok(files.includes(a.path),a.id);
