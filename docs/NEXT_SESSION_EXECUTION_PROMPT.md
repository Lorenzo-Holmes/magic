...
v1.11.0、v1.12.0、v2.0.0 已形成独立绿色恢复点。用户随后要求参考 `lehuo9248/xianxia-idle-game` 重做玩法与视觉，确认“有风险但能挽回”，本地已完成 v2.1.0「山海行旅」验收：172 项单测、6000 局经典兼容回归、1200 局新行旅策略、534 次布局检查。

恢复时先读 `docs/REDESIGN-v2.1.0.md`、`docs/ART-v2.1.0.md`、`docs/ACCEPTANCE-v2.1.0.md` 与 `release/push-v2.1.0.json`（若存在），并核对远端实际 HEAD。提交和部署结果以真实发布记录为准，不因本段验收说明推断已经上线。当前旧发布基线为 `a442e50e92a6f894c66e423311497939bd502432`；不得回退覆盖更高已验证版本。

后续回归另加 `npm run test:journey`。新局默认行旅，旧存档保留原突破规则，可在人物页启用。美术为两幅本地 imagegen 水墨位图，人物保持匿名黑色打坐剪影。发布仍只使用当前最后一个绿色版本；Cloudflare 登录需使用正常授权，不采用临时账户替代目标站点。
