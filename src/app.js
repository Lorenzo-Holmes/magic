(function () {
  ...
  if (state) { meta = M.observe(meta, state); saveMeta(); }
  ...
  function talentCard(t, mode) {
    ...
    const label = `<div class="talent-meta"><span>${esc(t.path)}</span><span class="rarity">${D.RARITIES[t.rarity].name}品</span></div><h3>${esc(t.name)}</h3><p>${esc(t.description)}</p><span class="card-bottom">${mode === 'pick' ? '择此道 →' : selected ? '已纳入命格 ✓' : '点选此命'}</span>`;
  ...
  function talentsView() {
    ...
    return `<section><p class="eyebrow">第一步 · 窥探天命</p><div class="section-heading"><h2>此生命数，<br><em>择三而行。</em></h2><span class="count"><b>${state.selected.length}</b> / 3</span></div><p class="intro">八道先天气运，选择三道。品质并非唯一答案，能相互成就的天命更重要。</p>${inherited ? `<div class="trace-banner"><span>前世道痕 · ${esc(inherited.path)}</span><b>${esc(inherited.name)}</b><p>本次签池至少出现一道${esc(inherited.talentPath)}天命；重抽不会抹去这份影响。</p></div>` : ''}<div class="talent-grid">${state.offer.map(id => talentCard(find(D.TALENTS, id), 'select')).join('')}</div><div class="setup-actions">${button('reroll-opening', `再窥天命 · 余 ${state.openingRerolls} 次`, { classes: 'secondary', disabled: !state.openingRerolls })}${button('confirm-talents', `命已择定 →`, { classes: 'primary', disabled: state.selected.length !== 3 })}</div><p class="footnote">重抽会清空本次选择。每局至少一张玄品以上；轮回道痕只保证对应流派出现，不直接增加战力。</p></section>`;
  }
