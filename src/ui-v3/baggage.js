  ...
  V.register('BaggageWindow',{
    render(ctx){
      ...
      if(filter==='all'||filter==='equipment') for(const entry of state.equipment.inventory){
        ...
        rows.push(`<article class="v3-item-row" data-item-id="${esc(entry.uid)}"><div class="v3-item-icon rarity-${def?.rarity||0}">${A.img(A.item('equipment',def.id,entry.identified))}</div><div class="v3-item-copy"><div><span>${entry.identified?`${V.rarityName(def.rarity)}品 · ${G.SLOTS[def.slot]}`:'未鉴定'}</span>${worn?'<em>已穿戴</em>':''}</div><h3>${esc(name)}</h3><p>${esc(entry.identified?def.description:'灵光封存，鉴定后才显露器物本相。')}</p></div><div class="v3-item-actions">${actions.join('')}</div></article>`);
      }
      if(filter==='all'||filter==='pills') for(const recipe of K.PILLS){
        ...
        rows.push(`<article class="v3-item-row"><div class="v3-item-icon pill">${A.img(A.item('pills',recipe.id))}</div><div class="v3-item-copy"><div><span>丹药</span><em>持有 ${owned}</em></div><h3>${esc(recipe.name)}</h3><p>${esc(recipe.text)}</p></div><div class="v3-item-actions">${owned?button('craft-use','服用<small>消耗一枚</small>',{id:recipe.id,classes:'v3-item-action',disabled:!canPill}):'<span class="v3-empty-note">暂无</span>'}</div></article>`);
      }
      if(filter==='all'||filter==='materials') for(const material of K.MATERIALS){
        ...
        rows.push(`<article class="v3-item-row compact"><div class="v3-item-icon material">${A.img(A.item('materials',material.id))}</div><div class="v3-item-copy"><div><span>炼制材料</span><em>× ${n}</em></div><h3>${esc(material.name)}</h3></div></article>`);
      }
      if(filter==='all'||filter==='supplies'){
        const j=state.journey;
        rows.push(`<article class="v3-item-row compact"><div class="v3-item-icon supply">${A.img('item.rice')}</div><div class="v3-item-copy"><div><span>行装</span><em>${j.supplies}/12</em></div><h3>行粮</h3><p>远行途中按段消耗。</p></div></article>`);
        rows.push(`<article class="v3-item-row compact"><div class="v3-item-icon supply">${A.img('item.medicine')}</div><div class="v3-item-copy"><div><span>行装</span><em>× ${j.medicine}</em></div><h3>伤药</h3><p>用于处理旅途伤势。</p></div></article>`);
      }
      ...
      return `<section class="v3-window v3-baggage-window">${A.title(button,'行囊','纳物有序',`<span>装备 ${state.equipment.inventory.length}/${G.MAX_INVENTORY} · 器蕴 ${fmt(state.equipment.essence)}</span>`)}${tools}<div class="v3-category-tabs">${categories.map(([id,label])=>button('v3-bag-filter',label,{ui:true,id,classes:`v3-category-tab${filter===id?' active':''}`,pressed:filter===id})).join('')}</div>${prep}<div class="v3-baggage-scroll">${rows.join('')||`<div class="v3-empty-bag">${A.img('item.bag')}<p>此类行囊尚空。</p></div>`}</div></section>`;
    }
  });
