(function(root){
  ...
  V.register('BaggageWindow',{
    render(ctx){
      ...
      const tools=`<details class="v4-bag-tools"><summary>行囊事务</summary><div class="v3-bag-tools">${button('forge','炼器阁',{ui:true,classes:'v3-bag-tool'})}${button('equipment','装备总览',{ui:true,classes:'v3-bag-tool'})}${button('crafting','丹器详册',{ui:true,classes:'v3-bag-tool'})}${button('spirit-beast','灵兽',{ui:true,classes:'v3-bag-tool'})}</div></details>`;

      return `<section class="v3-window v3-baggage-window v4-baggage-window">${A.title(button,'行囊 · 随身宝匣','纳物有序',`<span>装备 ${state.equipment.inventory.length}/${G.MAX_INVENTORY} · 器蕴 ${fmt(state.equipment.essence)}</span>`)}<div class="v3-category-tabs">${categories.map(([id,label])=>button('v3-bag-filter',label,{ui:true,id,classes:`v3-category-tab${filter===id?' active':''}`,pressed:filter===id})).join('')}</div><div class="v4-bag-main"><div class="v4-treasure-grid" role="list" aria-label="宝匣物品">${cells||'<p class="v4-bag-empty">此类行囊尚空。</p>'}</div>${detail}</div>${prep}${tools}</section>`;
    }
  });
