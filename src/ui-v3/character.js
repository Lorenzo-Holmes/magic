  ...
  V.register('CharacterWindow',{
    render({state,D,E,G,button,esc,fmt}){
      ...
      return `<section class="v3-window v3-character-window">${A.img('scene.character','v3-character-bg','云海道台')}
        ${A.title(button,'人物 · 此世道身',realm.name,`<span>${appearance.name} · ${state.age} 岁 · 元气 ${state.vitality} · 战力 ${fmt(E.power(state))}</span>`)}
        ...
        <div class="v3-character-lower">${chooser}<div class="v3-character-links">${[['journal','命册'],['build','大道'],['spirit-beast','灵兽'],['sect','宗门']].map(([id,name])=>button(id,name,{ui:true,classes:'v3-character-link'})).join('')}</div><div class="v3-character-secondary">${[['life','人生'],['karma','因果'],['legacy','轮回'],['codex','图谱']].map(([id,name])=>button(id,name,{ui:true,classes:'v3-character-sub-link'})).join('')}</div><div class="v3-stat-ribbon" aria-label="基础属性">${[['根骨',stats.bone],['神识',stats.mind],['悟性',stats.insight],['气运',stats.luck]].map(([n,v])=>`<span><small>${n}</small><b>${esc(v)}</b></span>`).join('')}</div></div>
      </section>`;
    }
  });
