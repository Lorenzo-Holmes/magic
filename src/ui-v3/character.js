  ...
  V.register('CharacterWindow',{
    render({state,D,E,G,button,esc,fmt}){
      ...
      return `<section class="v3-window v3-character-window v4-character-window">${A.img('scene.character','v3-character-bg','云海道台')}
        ...
        <div class="v3-character-lower"><section class="v4-character-dossier" aria-label="此世仙籍">${facts.map(([k,v])=>`<div><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('')}</section>${chooser}<div class="v3-character-links">${[['journal','命册'],['build','大道'],['spirit-beast','灵兽'],['sect','宗门']].map(([id,name])=>button(id,name,{ui:true,classes:'v3-character-link'})).join('')}</div><details class="v4-character-archive"><summary>修行档案</summary><div class="v3-character-secondary">${[['life','人生'],['karma','因果'],['legacy','轮回'],['codex','图谱']].map(([id,name])=>button(id,name,{ui:true,classes:'v3-character-sub-link'})).join('')}</div></details><div class="v3-stat-ribbon" aria-label="基础属性">${[['根骨',stats.bone],['神识',stats.mind],['悟性',stats.insight],['气运',stats.luck]].map(([n,v])=>`<span><small>${n}</small><b>${esc(v)}</b></span>`).join('')}</div></div>
      </section>`;
    }
  });
